...
class HelperGoldenFileComparator extends LocalFileComparator {
  ...
  static final _goldenFilePathRegex = RegExp(r'.+_v\d+\.png');
  ...
  Future<void> update(Uri golden, Uint8List imageBytes) async {
    final goldenFile =
        File(path.join(path.fromUri(basedir), path.basename(golden.path)));
...
Actual    : ${goldenFile.path}""");
    ...
    if (_forceUpdateGolden || !(await goldenFile.exists())) {
      await super.update(Uri.file(path.basename(golden.path)), imageBytes);
    } else {
      debugPrint('''\u001b[33m
